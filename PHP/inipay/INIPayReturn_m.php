					...
					if($_REQUEST["P_STATUS"] == "00"){
						...
						if ($result) {
							if(mysqli_affected_rows($connect) > 0 ){
								...
								$sql2 = "Insert into tbl_payment(user_idx, Payment_method, payment, reg_date, Payment_status, t_id) value($user_idx, 1, $pay_point, now(), 0, '$P_TID')";
								$result2 = mysqli_query($connect, $sql2);
						
							//	echo "<script>alert('Rose가 변경되었습니다.');location.href='admin1_2.php?user_idx=$user_idx';</script>";
							}else{
							//	echo "<script>alert('Rose 변경에 실패했습니다.');location.href='admin1_2.php?user_idx=$user_idx';</script>";
							}
